...
function InspectionSidebar({ node, isOpen, onClose, onNavigate, graphData }) {
  ...
  const owner = graphData?.meta?.owner
  const repo = graphData?.meta?.repo?.replace(/\.git$/, '')
  const githubFileUrl = owner && repo && node?.id
    ? `https://github.com/${owner}/${repo}/blob/main/${node.id}`
    : null
